...
if ( is_readable( $pow_dropin ) && filesize( $pow_dropin ) <= 16384 ) {
    ...
    if ( is_string( $pow_dropin_contents ) && false !== strpos( $pow_dropin_contents, 'Proof of Work Captcha managed advanced-cache drop-in' ) ) {
        wp_delete_file( $pow_dropin );
    }
}
...
if ( is_readable( $pow_runtime_config ) ) {
    ...
    if ( is_string( $pow_config_contents ) && false !== strpos( $pow_config_contents, 'Proof of Work Captcha generated runtime configuration' ) ) {
        wp_delete_file( $pow_runtime_config );
    }
}
...
delete_option( 'pow_interaction_mode' );
delete_option( 'pow_debug_progress' );
...
delete_option( 'pow_early_protection_config_version' );
